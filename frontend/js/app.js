...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function showView(name) {
    ...
    $$('#header nav button[data-view]').forEach(b => b.classList.toggle('active', b.dataset.view === name));
  }
  ...
  async function cargarDashboard() {
    ...
    grid.innerHTML = '<p style="color:var(--text-light)">Cargando...</p>';
    try {
      ...
      grid.innerHTML = `
        <div class="stat-card purple"><div class="stat-icon">📚</div><div class="stat-value">${totales.total_tutorias}</div><div class="stat-label">Tutorías</div></div>
        <div class="stat-card blue"><div class="stat-icon">📋</div><div class="stat-value">${totales.total_reservas}</div><div class="stat-label">Reservas</div></div>
        <div class="stat-card green"><div class="stat-icon">✅</div><div class="stat-value">${asistencia.porcentaje_asistencia}%</div><div class="stat-label">Asistencia</div></div>
        <div class="stat-card yellow"><div class="stat-icon">⏳</div><div class="stat-value">${pendientes}</div><div class="stat-label">Pendientes</div></div>
        <div class="stat-card red"><div class="stat-icon">👥</div><div class="stat-value">${totales.total_estudiantes}</div><div class="stat-label">Estudiantes</div></div>
      `;
    } catch { grid.innerHTML = '<p class="error-msg">Error al cargar dashboard</p>'; }
    ...
    try {
      ...
      if (filtradas.length === 0) {
        topDiv.innerHTML = '<p style="color:var(--text-light);font-size:0.9rem">Sin datos aún</p>';
      } else {
        topDiv.innerHTML = filtradas.map(t => `
          <div style="display:flex;justify-content:space-between;padding:0.6rem 0;border-bottom:1px solid var(--border)">
            <span>${escHtml(t.tema)} <span style="color:var(--text-light);font-size:0.8rem">— ${escHtml(t.docente_nombre)}</span></span>
  ...
  $('#formTutoria').addEventListener('submit', async (e) => {
    ...
    try {
      if (id) { await API.actualizarTutoria(+id, data); toast('Tutoría actualizada'); }
      else { await API.crearTutoria(data); toast('Tutoría creada'); }
  ...
  async function cargarTutorias(showSpinner = true) {
    ...
    if (showSpinner) div.innerHTML = '<p style="color:var(--text-light);text-align:center;padding:2rem">Cargando...</p>';
    try {
      const list = await API.getTutorias();
      const user = API.getUser();
      if (list.length === 0) {
        div.innerHTML = '<div class="empty-state"><div class="empty-icon">📚</div><p>No hay tutorías disponibles.</p></div>';
        return;
      }
        ...
        return `
          <div class="tutoria-card">
            ...
            <div class="actions">
              ...
              ${isOwner ? `<button class="btn secondary small btn-editar-tutoria" data-id="${t.id_tutoria}">✏️</button>` : ''}
              ${isOwner && t.estado !== 'Ocupado' ? `<button class="btn danger small btn-eliminar-tutoria" data-id="${t.id_tutoria}">🗑️</button>` : ''}
            </div>
          </div>
        `;
      }).join('');
      ...
      div.querySelectorAll('.btn-reservar').forEach(b => b.addEventListener('click', async () => {
        try { const d = await API.crearReserva(+b.dataset.id, user.id_usuario); toast(d.mensaje); cargarTutorias(false); }
        catch (err) { toast(err.message, 'error'); }
      }));
      ...
      div.querySelectorAll('.btn-eliminar-tutoria').forEach(b => b.addEventListener('click', async () => {
        ...
        if (!ok) return;
        try { await API.eliminarTutoria(+b.dataset.id); toast('Tutoría eliminada', 'success'); cargarTutorias(false); }
        catch (err) { toast(err.message, 'error'); }
      }));
  ...
  async function cargarReservas() {
    ...
    div.innerHTML = '<p style="color:var(--text-light);text-align:center;padding:2rem">Cargando...</p>';
    try {
      const list = await API.getReservas();
      const user = API.getUser();
      if (list.length === 0) {
        div.innerHTML = '<div class="empty-state"><div class="empty-icon">📋</div><p>No hay reservas.</p></div>';
        return;
      }
      div.innerHTML = list.map(r => `
        <div class="reserva-card">
          ...
          <div class="actions">
            <span class="badge badge-${r.estado_asistencia === 'Asistio' ? 'asistio' : r.estado_asistencia === 'Falto' ? 'falto' : 'pendiente'}">${r.estado_asistencia === 'Asistio' ? '✅ Asistió' : r.estado_asistencia === 'Falto' ? '❌ Faltó' : '⏳ Pendiente'}</span>
      ...
      div.querySelectorAll('.sel-asistencia').forEach(s => s.addEventListener('change', async () => {
        try { await API.marcarAsistencia(+s.dataset.id, s.value); toast('Asistencia actualizada'); cargarReservas(); }
        catch (err) { toast(err.message, 'error'); }
      }));
      div.querySelectorAll('.btn-cancelar-reserva').forEach(b => b.addEventListener('click', async () => {
        ...
        if (!ok) return;
        try { await API.request('PUT', `/reservas/${b.dataset.id}/cancelar`); toast('Reserva cancelada', 'warning'); cargarReservas(); cargarTutorias(false); }
        catch (err) { toast(err.message, 'error'); }
      }));
    } catch (err) { div.innerHTML = `<p class="error-msg">${err.message}</p>`; }
  }
    ...
    try {
      ...
      tbody.innerHTML = list.map(u => `
        <tr>
          ...
          <td>
            <button class="btn secondary small editar-usuario" data-id="${u.id_usuario}">✏️</button>
            <button class="btn danger small eliminar-usuario" data-id="${u.id_usuario}">🗑️</button>
          </td>
        </tr>
      `).join('');
      ...
      tbody.querySelectorAll('.eliminar-usuario').forEach(b => b.addEventListener('click', async () => {
        ...
        if (!ok) return;
        try { await API.eliminarUsuario(+b.dataset.id); toast('Usuario eliminado', 'success'); cargarUsuarios(); }
        catch (err) { toast(err.message, 'error'); }
      }));
  ...
  $('#formUsuario').addEventListener('submit', async (e) => {
    ...
    try {
      if (id) { await API.actualizarUsuario(+id, data); toast('Usuario actualizado'); }
      else { await API.crearUsuario(data); toast('Usuario creado'); }
